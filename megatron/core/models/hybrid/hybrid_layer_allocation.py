...
def _get_contiguous_layer_pattern(main_pattern: str) -> str:
    """Return the full decoder layer pattern with pipe separators removed."""
    if not main_pattern:
        return ''
    ...
    if len(segments) > 1:
        log_single_rank(
            logger,
            logging.WARNING,
            "OctoPipe pipeline segment selection ignores pipe ('|') separators "
            "in hybrid_layer_pattern; stage boundaries come from partition.txt.",
        )
    return ''.join(segments)


def select_octopipe_pipeline_segment(
    main_pattern: str,
    vp_stage: int,
    octopipe_config: dict,
    pp_group: Optional[torch.distributed.ProcessGroup] = None,
    tp_group: Optional[torch.distributed.ProcessGroup] = None,
    dp_cp_group: Optional[torch.distributed.ProcessGroup] = None,
) -> Tuple[List[str], int]:
    """Select the pipeline segment for an OctoPipe logical stage.

    Uses ``partition`` and ``layer_idx_offset`` from the OctoPipe config to slice
    the hybrid layer pattern. ``vp_stage`` is the OctoPipe stage id (sid), not the
    physical pipeline-parallel rank.

    Args:
        main_pattern: Main decoder pattern from ``--hybrid-layer-pattern``.
        vp_stage: OctoPipe logical stage id (sid).
        octopipe_config: OctoPipe config dict from ``get_octopipe_config()``.
        pp_group: Pipeline parallel process group, used for logging only.
    ...
    Returns:
        Tuple of (layer_type_list, layer_offset) for this OctoPipe stage.

    Raises:
        ValueError: If ``vp_stage`` is out of range or pattern length does not match
            the OctoPipe partition.
    """
    partition = octopipe_config["partition"]
    layer_idx_offset = octopipe_config["layer_idx_offset"]

    if vp_stage < 0 or vp_stage >= len(partition):
        raise ValueError(
            f"OctoPipe stage id {vp_stage} is out of range for partition "
            f"with {len(partition)} stages: {partition}."
        )
    ...
    expected_total = sum(partition)
    if num_layers != expected_total:
        raise ValueError(
            f"Hybrid layer pattern length ({num_layers}) does not match "
            f"OctoPipe partition total ({expected_total}): {partition}."
        )

    offset = layer_idx_offset[vp_stage]
    count = partition[vp_stage]
    if offset + count > num_layers:
        raise ValueError(
            f"OctoPipe partition slice for stage {vp_stage} "
            f"(offset={offset}, count={count}) exceeds pattern length "
            f"({num_layers})."
        )
    ...
    log_on_each_pipeline_stage(
        ...
        f"layer_offset={offset} (octopipe partition={partition})",
        tp_group=tp_group,
...
def select_pipeline_segment(
    ...
    if octopipe_enabled():
        if vp_stage is None:
            raise ValueError(
                "vp_stage (OctoPipe stage id) must be provided when --octopipe is "
                "enabled and hybrid layer pattern has no pipe ('|') separators."
            )
        return select_octopipe_pipeline_segment(
            main_pattern,
            vp_stage,
            get_octopipe_config(),
            pp_group=pp_group,
