...
class Float16Module(MegatronModule):
    ...
    def forward(self, *inputs, fp32_output=True, **kwargs):
        ...
        # Prefer explicit pre/post flags set at model build time (covers OctoPipe, which
        # stores logical stage id in vp_stage rather than VPP index).
        if hasattr(self.module, "pre_process"):
            cast_inputs = self.module.pre_process
        else:
            cast_inputs = is_vp_first_stage(self.vp_stage, self.vp_size) and is_pp_first_stage(
                pp_group
            )
        ...
        if hasattr(self.module, "post_process"):
            cast_outputs = self.module.post_process and fp32_output is True
        else:
            cast_outputs = (
