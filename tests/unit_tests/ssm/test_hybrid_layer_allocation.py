...
from megatron.core.models.hybrid.hybrid_layer_allocation import (
    ...
    pattern_from_ratios,
    select_octopipe_pipeline_segment,
...
@pytest.mark.internal
class TestSelectOctopipePipelineSegment:
    """Tests for select_octopipe_pipeline_segment."""

    @staticmethod
    def _octopipe_config(partition):
    ...
    def test_asymmetric_partition(self):
        pattern = "MMMMMM"
        config = self._octopipe_config([2, 3, 1])
        with patch(
            'megatron.core.models.hybrid.hybrid_layer_allocation.log_on_each_pipeline_stage'
        ):
            layers0, off0 = select_octopipe_pipeline_segment(pattern, 0, config)
            layers1, off1 = select_octopipe_pipeline_segment(pattern, 1, config)
            layers2, off2 = select_octopipe_pipeline_segment(pattern, 2, config)
        assert layers0 == ['M', 'M']
        assert off0 == 0
        assert layers1 == ['M', 'M', 'M']
        assert off1 == 2
        assert layers2 == ['M']
        assert off2 == 5

    def test_all_stages_reconstruct_pattern(self):
        pattern = "MMMMMM"
        partition = [2, 3, 1]
        config = self._octopipe_config(partition)
        reconstructed = []
        with patch(
            'megatron.core.models.hybrid.hybrid_layer_allocation.log_on_each_pipeline_stage'
        ):
            for sid in range(len(partition)):
                layers, offset = select_octopipe_pipeline_segment(pattern, sid, config)
                assert offset == len(reconstructed)
                reconstructed.extend(layers)
        assert reconstructed == list(pattern)

    def test_pattern_length_mismatch_raises(self):
        config = self._octopipe_config([2, 2])
        with patch(
            'megatron.core.models.hybrid.hybrid_layer_allocation.log_on_each_pipeline_stage'
        ):
            with pytest.raises(ValueError, match="does not match OctoPipe partition total"):
                select_octopipe_pipeline_segment("MMMM", 0, config)

    def test_invalid_stage_raises(self):
        config = self._octopipe_config([2, 2])
        with pytest.raises(ValueError, match="out of range"):
            select_octopipe_pipeline_segment("MMMM", 2, config)

    def test_select_pipeline_segment_dispatches_to_octopipe(self):
        pattern = "MMMMMM"
        config = self._octopipe_config([2, 3, 1])
        mock_group = object()
        with (
            patch('megatron.training.global_vars.octopipe_enabled', return_value=True),
            patch('megatron.training.global_vars.get_octopipe_config', return_value=config),
            patch('torch.distributed.get_rank', return_value=1),
            patch('torch.distributed.get_world_size', return_value=4),
            patch('megatron.core.models.hybrid.hybrid_layer_allocation.log_on_each_pipeline_stage'),
        ):
            layers, offset = select_pipeline_segment(
                pattern, pp_group=mock_group, vp_stage=1
            )
        assert layers == ['M', 'M', 'M']
        assert offset == 2
